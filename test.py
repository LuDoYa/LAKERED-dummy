...
def inference_dataset(args):
    ...
    all_masks = sorted(glob.glob(os.path.join(args.dataset_root, 'validation/masks', args.data_type + "_*")))
    all_images = sorted(glob.glob(os.path.join(args.dataset_root, 'validation/images', args.data_type + "_*")))

    # Calculate 10% of the data
    num_items = len(all_masks)
    num_to_select = int(num_items * 0.01)

    # Select the first 10% of the data
    masks = all_masks[:num_to_select]
    images = all_images[:num_to_select]
        ...
        with model.ema_scope():
            for image_pth, mask_pth in tqdm(zip(images, masks)):
                try:
                    ...
                    img.save(os.path.join(args.dst_root, imgname+'.jpg'))
                    ...
                    if args.isSaveMask:
                        ...
                        mask.save(os.path.join(args.dst_root, imgname+'.png'))       
