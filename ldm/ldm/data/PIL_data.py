...
class InpaintingTrain_ldm(Dataset):
    def __init__(self, size, data_root, config=None):
        ...
        self.config = config or OmegaConf.create()
        self.data_root=data_root
        self.img_paths = sorted(_list_image_files_recursively(str(Path(self.data_root) / 'images')))
        self.mask_paths = sorted(_list_image_files_recursively(str(Path(self.data_root) / 'masks')))
